...
export function StudentInforUploadPage ({ changePage }) {
    ...
    const handleAddRow = () => {
        const newRow = {
            id: studentId, // 使用 studentId 作为 id
            name: name,
            dob: dob,
            age: parseInt(age),
            studentId: studentId
        ...
        // 执行提交操作
        axios.post('http://localhost:5002/api/auth/studentInfo', newRow, { withCredentials: true })
            .then((res) => {
    ...
    const handleDeleteRow = (studentId) => {
        // 执行删除操作
        axios.delete(`http://localhost:5002/api/auth/deleteStudentInfo/${studentId}`, { withCredentials: true })
        ...
        axios
            ...
            .then((res) => {
                if (res.data && res.data.students) {
                    const receivedData = res.data.students.map((student, index) => {
                        return { id: index, ...student }
                    })
                    setTableRows(receivedData)
                }
            })
            .catch((err) => console.log(err))
    }, [])
